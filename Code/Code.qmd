...
data <- data %>% dplyr::select(
  -c('mat_race','record_id','center', 'Death', 'Trach'))
```
...
cont <- data %>% select(-c("ventilation_support_level_modified.44",
...
ggplot(data = cor_df, aes(x = Var1, y = Var2)) + 
  ...
  coord_fixed() +
  labs(x = "", y = "", title=
         "Correlation Matrix Between Predictors and Outcome") 
```
...
data <- data %>% dplyr::select(-c('any_surf'))
...
for (i in 1:5){
  data_36[[i]] = data_imp[[i]] %>% dplyr::select(-c(wk44_vars))
}
```
...
ridge <- function(X,Y) {
  ...
  # Ridge model
  ridge_mod <- cv.glmnet(X, Y, nfolds = 10, family = 'binomial',
                         foldid = folds, alpha = 0)
  
  # Refit using lambda.min
  ridge_mod <- glmnet(X, Y, nfolds = 10, family = 'binomial',
                         lambda=ridge_mod$lambda.min, alpha = 0)
...
model_data <- function(all_imputations, p) {
  ...
  # selection and fit 3 Logistic Models: Lasso, Ridge, Forward Stepwise
  ...
  avg_fwd_coefs <- matrix(0, ncol = length(all_imputations), 
                          nrow = p)
  avg_ridge_coefs <- matrix(0, ncol = length(all_imputations), 
                          nrow = p)
  ...
  for (i in 1:length(all_imputations)) {
    ...
    ridge_coefs <- ridge(x_train, y_train)
    # Predictions
    log_odds_ridge <- x_test %*% ridge_coefs
    pred_ridge <- as.numeric(1 / (1 + exp(-log_odds_ridge)))
    ...
    avg_ridge_coefs[,i] <- ridge_coefs
    avg_lasso_coefs[,i] <- lasso_coefs
    
    # Calculate evaluation metrics for each model
    for (model_name in c("fwd", "ridge", "lasso")) {
      pred <- get(paste0("pred_", model_name))

      # Accuracy
      eval_metrics$accuracy[[model_name]][i] <- mean(round(pred) == y_test)

      # F1-Score, Sensitivity, and Specificity
      confusion_matrix <- table(y_test, round(pred))
